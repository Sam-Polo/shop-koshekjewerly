...
// Сердце от дизайнеров: два файла — "heart not hover.svg" (контур, дырка внутри через
// два вложенных контура в одном path) и "heart hover.svg" (сплошная заливка). Оригиналы
// зашиты на #B01E4A — заменили на currentColor, чтобы сохранить уже работающий паттерн
// цвета (белый контур поверх фото товара, розовая заливка когда товар в избранном) через
// ту же CSS-переменную --fav-color, что и остальные состояния кнопки.
const HEART_OUTLINE = 'M22.3445 2.01364C22.356 2.01196 22.3675 2.01027 22.3791 2.00859C25.2491 1.86993 27.8214 3.41475 28.0677 6.29838C28.1518 7.3651 28.04 8.43804 27.7385 9.46547C26.4776 13.888 21.9331 19.186 18.9066 22.7322L17.4405 24.4272C17.1959 24.7055 16.6009 25.4837 16.1989 25.5435C16.0694 25.5629 15.5683 25.3881 15.4203 25.3363C13.3328 24.6053 11.4149 23.4158 9.57921 22.2076C9.0589 21.8637 8.0558 21.1763 7.61771 20.7715C5.43078 19.0504 3.64703 16.8684 2.68761 14.2489C1.92994 12.1802 1.64834 9.63237 2.62731 7.5881C3.1243 6.53269 4.02401 5.71545 5.12909 5.3156C8.509 4.12784 12.3347 6.96117 13.4383 10.0583C14.445 6.38269 16.863 3.19118 20.7538 2.23342C21.2846 2.10278 21.8037 2.07023 22.3445 2.01364ZM7.14154 18.9187C7.7079 19.5129 8.70889 20.2791 9.39154 20.7627C10.9069 21.8363 12.6564 22.9351 14.339 23.729C14.8909 23.9877 15.4584 24.2126 16.0383 24.4024C16.6049 23.7009 17.2707 22.9676 17.867 22.2763C19.3381 20.5868 20.7606 18.8564 22.1328 17.0873C24.3521 14.191 27.3319 10.1043 26.9498 6.23693C26.8658 5.38439 26.4269 4.55091 25.7502 4.03091C24.9326 3.36968 23.635 3.03232 22.5908 3.09388C20.4414 3.13262 18.5299 4.08757 17.0617 5.59868C15.5499 7.15465 14.8516 9.01247 14.2532 11.0442C14.1409 11.4254 13.963 11.9356 13.4848 11.9462C12.9015 11.959 12.7736 11.2082 12.5805 10.8023C12.4901 10.5802 12.4005 10.3632 12.2994 10.1452C11.3353 8.06581 9.22705 6.16526 6.81678 6.13095C4.29851 6.09509 3.10355 8.18046 3.0953 10.45C3.08533 13.1963 4.28004 15.7485 6.06412 17.8143C6.37164 18.1689 6.77657 18.6258 7.14154 18.9187Z'
const HEART_SOLID = 'M22.3791 2.00859L22.3445 2.01364C21.8037 2.07023 21.2846 2.10278 20.7538 2.23342C16.863 3.19118 14.445 6.38269 13.4383 10.0583C12.3347 6.96117 8.509 4.12784 5.12909 5.3156C4.02401 5.71545 3.1243 6.53269 2.62731 7.5881C1.64834 9.63237 1.92994 12.1802 2.68761 14.2489C3.64703 16.8684 5.43078 19.0504 7.61771 20.7715C8.0558 21.1763 9.0589 21.8637 9.57921 22.2076C11.4149 23.4158 13.3328 24.6053 15.4203 25.3363C15.5683 25.3881 16.0694 25.5629 16.1989 25.5435C16.6009 25.4837 17.1959 24.7055 17.4405 24.4272L18.9066 22.7322L18.9875 22.6375C22.0135 19.0921 26.4887 13.8487 27.7385 9.46547C28.04 8.43804 28.1518 7.3651 28.0677 6.29838C27.8214 3.41475 25.2491 1.86993 22.3791 2.00859Z'

// filled — товар уже в избранном. viewBox не квадратный (30×28) — preserveAspectRatio
// по умолчанию вписывает без искажений, поэтому width=height=size остаётся безопасным
const IconHeart = ({ size = 22, filled = false }: { size?: number; filled?: boolean }) => (
  <svg width={size} height={size} viewBox="0 0 30 28" fill="none" aria-hidden="true">
    <path d={filled ? HEART_SOLID : HEART_OUTLINE} fill="currentColor" />
  </svg>
)
...
// FAQ по умолчанию — показывается, пока не загрузился (или недоступен) список из админки.
// В тексте ответа: каждая строка = отдельный абзац; [[текст]] = кликабельная ссылка на менеджера.
const DEFAULT_FAQ: Array<{ question: string; answer: string }> = [
  {
    question: 'Как долго ждать?',
    answer: '— Изготовление и сборка занимает 2-3 дня. Изделия из special collection (ягоды) около 4-6 дней.',
  },
  {
    question: 'Как происходит доставка?',
    answer: '— По Москве и МО: 350₽\n— По России: 500₽\n— СНГ: 650₽\n— Европа: 1500₽',
  },
  {
    question: 'Международная доставка: пошлины и налоги',
    answer: '— Таможенные пошлины и налоги (VAT) страны назначения не включены в стоимость заказа и доставки.\n— Они оплачиваются получателем при получении посылки по правилам страны назначения.',
  },
  {
    question: 'Условия возврата товара и денежных средств',
    answer: '— Возврат товара надлежащего качества возможен в течение 14 дней с момента получения заказа.\n— Товар должен быть в оригинальном виде, с бирками, без следов использования, в оригинальной упаковке.\n— Возврат денежных средств осуществляется на ту же карту, с которой была произведена оплата, в течение 10 рабочих дней.\n— При обнаружении брака или несоответствия описанию товар можно вернуть в течение гарантийного срока.\n— Для оформления возврата свяжитесь с [[менеджером]]',
  },
]

// строка ответа FAQ → абзац; [[текст]] превращается в ссылку на менеджера
const renderFaqLine = (line: string, key: number) => {
  const parts = line.split(/\[\[(.+?)\]\]/g)
  return (
    <p key={key}>
      {parts.map((part, i) => (i % 2 === 1 ? <ManagerLink key={i}>{part}</ManagerLink> : part))}
    </p>
  )
}

const AboutUsModal = ({ onClose }: { onClose: () => void }) => {
  const [faq, setFaq] = useState<Array<{ question: string; answer: string }>>(DEFAULT_FAQ)
  ...
  useEffect(() => {
    ...
    fetch(`${apiUrl}/api/faq`)
      ...
      .then(data => {
        if (Array.isArray(data?.items) && data.items.length > 0) setFaq(data.items)
      })
      .catch(() => {}) // недоступен бэкенд — остаёмся на дефолтном списке
  }, [])
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content" onClick={e => e.stopPropagation()}>
        ...
        {faq.map((item, i) => (
          <AccordionItem key={`${i}-${item.question}`} question={item.question}>
            {item.answer.split('\n').map((line, j) => renderFaqLine(line, j))}
          </AccordionItem>
        ))}
      </div>
    </div>
...
//
// ⚠️ ВТОРАЯ КОПИЯ этого текста — `admin/frontend/src/rules-default.ts`: там он
// служит стартовой точкой редактирования, пока лист `rules` пуст. Разделить копии
// нельзя (админка и мини-апп — разные пакеты), при правке менять ОБА файла.
//
  ...
  return (
    <>
      <div className="account-card">
        {firstName && <p className="account-card__greeting">Привет, {firstName}</p>}
        {totalOrders > 0 && (
