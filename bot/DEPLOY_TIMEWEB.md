...
**Проверка .env файла:**
```bash
# проверь что файл создан и содержит правильные данные
cat .env

# убедись что токен правильный (без пробелов, без кавычек)
# формат: TG_BOT_TOKEN=8424425850:AAFZNdMJSu8EgfbWke0EFkuUsmwMNW2_eJ8
```

**Важно:** 
- Токен должен быть БЕЗ пробелов в начале и конце
- Токен должен быть БЕЗ кавычек (не `TG_BOT_TOKEN="токен"`, а `TG_BOT_TOKEN=токен`)
- Файл `.env` должен быть в директории `/opt/bot/bot/` (там же где `package.json`)
...
# или загрузи новые файлы через SFTP/SCP

# устанавливаем зависимости (если добавились новые)
npm install
...
**Важно:**
- Если изменился `.env` файл - отредактируй его через `nano .env` и перезапусти бота
- Если добавились новые npm пакеты - выполни `npm install` перед сборкой
- PM2 автоматически перезапустит бота с новым кодом
