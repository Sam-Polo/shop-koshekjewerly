 ...
 *
 * Без use-online-balance=true партия идёт по «классической» схеме — оплата при
 * физической сдаче в отделении. Флаг переключает её на списание с онлайн-баланса
 * аккаунта; баланс не пополнен, и в ЛК скачивание документов по такой партии
 * ругается «недостаточно средств». Флаг добавляли 03.07.2026 по совету
 * техподдержки в попытке починить печатные формы — на формы он НЕ влияет
 * (проверено матрицей 04.08.2026), поэтому убран как ложный след.
 *
 * Доступность печатных форм определяется видом отправления, а не этим флагом:
 * EMS формы отдаёт, SMALL_PACKET (мелкий пакет, текущий прод) — 403.
 * Подробности: docs/integrations/pochta-ems.md.
 */
