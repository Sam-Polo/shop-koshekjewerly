...
export type Order = {
  ...
  orderData: {
    items: Array<{
      ...
      /**
       * выбранная покупателем опция в виде «Размер: 17» (название группы + значение).
       * Хранится готовой строкой: на цену не влияет, нужна менеджеру при отгрузке.
       * Один и тот же товар с разными опциями — разные позиции заказа.
       */
      option?: string
    }>
    fullName: string
