...
export type OrderCustomerStatus =
  | 'Принят' | 'В сборке' | 'В пути' | 'Отправлен' | 'Уже у вас' | 'Не вручен - возврат'
 ...
 *
 * «Не вручен - возврат» стоит на одной ступени с «Уже у вас» по той же причине: обе метки
 * конечные и взаимоисключающие, и побеждает пришедшая первой. Это защищает от реального
 * сценария: невручённая посылка едет обратно, и СДЭК может прислать по ней ещё один
 * DELIVERED — уже про вручение ОТПРАВИТЕЛЮ. При меньшем ранге такой вебхук перебил бы
 * возврат на «Уже у вас», и покупатель прочитал бы, что посылка у него.
 */
