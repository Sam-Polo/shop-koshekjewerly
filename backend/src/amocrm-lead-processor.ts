...
/**
 * Этап воронки → статус заказа для покупателя в ЛК.
 *
 * «Уже у вас» здесь намеренно отсутствует: доставку подтверждает только СДЭК своим
 * вебхуком DELIVERED. Этапы CRM двигает менеджер, и назначать по ним «уже у вас» —
 * значит рисковать сказать покупателю, что посылка у него, когда она ещё едет.
 *
 * ВОЗВРАЩЕН (86423894) и Закрыто (143) не отображаются: ни в один из четырёх статусов
 * они не ложатся, а показывать возврат как «В пути» было бы враньём. Заказ просто
 * остаётся с последним статусом.
 */
export const CUSTOMER_STATUS_BY_STAGE: Record<number, 'Принят' | 'В сборке' | 'В пути'> = {
  86423882: 'Принят',    // Неразобранное
  86486222: 'Принят',    // ПРИОРИТЕТНЫЙ ЗАКАЗ
  86423886: 'Принят',    // НОВЫЙ, ЖДЕТ ОТПРАВКИ
  86584502: 'Принят',    // САМОВЫВОЗ
  86486582: 'В сборке',  // В РАБОТЕ
  86486586: 'В сборке',  // Собран
  86462242: 'В пути',    // Отправлен
  142:      'В пути',    // Завершён (системный «Успешно реализовано»)
}

/** статус для покупателя по этапу воронки; null — этап не отображаем */
export function customerStatusForStage(statusId: number): 'Принят' | 'В сборке' | 'В пути' | null {
  return CUSTOMER_STATUS_BY_STAGE[statusId] ?? null
}
